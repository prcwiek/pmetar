...
metar_location <- function(x) {
  if(nchar(x) == 0){
    ...
    if(sum(str_count(mst, pattern = x %R% one_or_more(SPC) %R% one_or_more(DGT) %R% one_or_more(SPC)
      ...
      m_t <- str_extract(mst, pattern = x %R% one_or_more(SPC) %R% one_or_more(WRD) %R% one_or_more(SPC) %R% one_or_more(DGT) %R% one_or_more(SPC)
                         %R% one_or_more(DGT) %R% SPC %R% one_or_more(DGT) %R% or("N", "S")
                         %R% one_or_more(SPC) %R% one_or_more(DGT) %R% SPC %R% one_or_more(DGT)
                         %R% or("E", "W") %R% one_or_more(SPC) %R% one_or_more(DGT))
    }
