...
metar_temp <- function(x){
  out <- c(1:length(x))
  out[1:length(x)] <- NA
  fT <- str_detect(x, pattern = "(M\\d{2}/\\d{2}|M\\d{2}/M\\d{2})")
  out[fT] <- as.numeric(str_sub(str_extract(x[fT], pattern = "(M\\d{2}/\\d{2}|M\\d{2}/M\\d{2})"), 2, 3)) * -1
  fT <- str_detect(x, pattern = "(\\d{2}/\\d{2}|\\d{2}/M\\d{2})")
  out[fT] <- as.numeric(str_sub(str_extract(x[fT], pattern = "(\\d{2}/\\d{2}|\\d{2}/M\\d{2})"), 1, 2))
