...
test_that("Check if airport code has 4 character", {
  ...
  expect_equal(stringr::str_length(metar_airport(x9)), 4)
})
...
test_that("Check airports ICAO codes", {
  ...
  expect_equal(metar_airport(x), c("EPWA", "CYUL", "LEMD", "KDCA", "KDCA", "CYWG", "KJFK", "NFTL", "NFTL"))
  expect_equal(metar_airport(dx$metar), c("EPWA", "CYUL", "LEMD", "KDCA", "KDCA", "CYWG", "KJFK", "NFTL", "NFTL"))
})
...
test_that("Incorrect METAR reports", {
  ...
  expect_equal(metar_airport(x13), "Incorrect METAR!")
})
