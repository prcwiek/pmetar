...
test_that("Check temperature", {
  expect_equal(metar_temp(x1), 3)
  expect_equal(metar_temp(x2), 1)
  expect_equal(metar_temp(x3), 4)
  expect_equal(metar_temp(x4), 13.9)
  expect_equal(metar_temp(x5), -13.9)
  expect_equal(metar_temp(x6), -5)
  expect_equal(metar_temp(x7), 23)
  expect_equal(metar_temp(x), c(3, 1, 4, 13.9, -13.9, -5, 23))
  expect_equal(metar_temp(dx$metar), c(3, 1, 4, 13.9, -13.9, -5, 23))
})
...
x10 <- "202001010851 METAR KEWR 010851Z 27010KT 10SM FEW030 BKN070 BKN100 BKN210 04/M03 A2969 RMK SLP054 T40391033 52012"

test_that("Incorrect METAR reports", {
  expect_equal(is.na(metar_temp(x8)), TRUE)
  expect_equal(is.na(metar_temp(x9)), TRUE)
  expect_equal(metar_temp(x10), 4)
})

dx <- data.frame(metar = c(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10))

test_that("Correct and incorrect METAR reports", {
  expect_equal(metar_temp(dx$metar),  c(3, 1, 4, 13.9, -13.9, -5, 23, NA, NA, 4))
})
