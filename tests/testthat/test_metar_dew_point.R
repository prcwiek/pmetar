...
test_that("Check dew point, check = FALSE", {
  ...
  expect_equal(metar_dew_point(x, check = FALSE), c(-1, -6, -3, 12.2, -12.2, -8, 20))
  expect_equal(metar_dew_point(dx$metar, check = FALSE), c(-1, -6, -3, 12.2, -12.2, -8, 20))
})
...
test_that("Check dew point, check = TRUE", {
  ...
  expect_equal(metar_dew_point(x, check = TRUE), c(-1, -6, -3, 12.2, -12.2, -8, 20))
  expect_equal(metar_dew_point(dx$metar, check = TRUE), c(-1, -6, -3, 12.2, -12.2, -8, 20))
})
...
x10 <- "202001010851 METAR KEWR 010851Z 27010KT 10SM FEW030 BKN070 BKN100 BKN210 04/N03 A2969 RMK SLP054 T003980330 52012"
...
test_that("Correct and incorrect METAR reports", {
  expect_equal(metar_dew_point(dx$metar),  c(-1, -6, -3, 12.2, -12.2, -8, 20, NA, NA, NA))
})
