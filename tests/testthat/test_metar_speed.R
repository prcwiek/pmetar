...
test_that("Incorrect METAR reports, check = FALSE", {
  expect_equal(metar_speed(x8, check = FALSE), 0)
  expect_equal(metar_speed(x9, check = FALSE), 0)
  expect_equal(metar_speed(x10, check = FALSE), 0)
  expect_equal(metar_speed(x8, metric = FALSE, check = FALSE), 0)
  expect_equal(metar_speed(x9, metric = FALSE, check = FALSE), 0)
  expect_equal(metar_speed(x10, metric = FALSE, check = FALSE), 0)
})
